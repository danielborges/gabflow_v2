...
def _hero(tenant: Tenant, starts_at: datetime, ends_at: datetime, styles: dict) -> list:
    office = (
        (tenant.visual_identity or {}).get("dadosInstitucionais", {}).get("nomeGabinete")
        or tenant.name
    )
    ...
    card = Table(
        [[title, [Paragraph(html_escape(office), styles["office"]), Paragraph(html_escape(identity), styles["identity"]), Spacer(1, 0.18 * cm), period]]],
...
def _summary(events: list[AgendaEvent], styles: dict) -> list:
    ...
    participants = {
        ...
        for event in events for participant in (event.participants or [])
    ...
    return [_section_title("Visão geral", "Capacidade e presença previstas para o período.", styles), table, Spacer(1, 0.45 * cm)]


def _metric_card(value: str, label: str, tone, background, styles: dict):
    table = Table(
        [[Paragraph(value, ParagraphStyle("MetricValue", parent=styles["metric_value"], textColor=tone)), Paragraph(html_escape(label), styles["metric_label"])]],
...
def _week_rhythm(events: list[AgendaEvent], starts_at: datetime, styles: dict) -> list:
    ...
    return [_section_title("Ritmo da semana", "Distribuição dos compromissos por dia.", styles), table, Spacer(1, 0.48 * cm)]
...
def _executive_highlights(events: list[AgendaEvent], styles: dict) -> list:
    ...
    insights = [
        ("PICO DA AGENDA", f"{_weekday_date(busiest)} concentra o maior volume." if busiest else "Semana sem compromissos registrados.", BLUE),
        ("PRESENÇA PARLAMENTAR", f"{len(representative)} compromisso(s) exigem atenção direta do parlamentar.", PURPLE),
        ("PREPARAÇÃO", f"{missing_location} sem local e {without_participants} sem participantes definidos.", AMBER if missing_location or without_participants else GREEN),
    ]
    cards = []
    for label, text, tone in insights:
        cards.append(Table(
            [[Paragraph(label, ParagraphStyle("InsightLabel", parent=styles["insight_label"], textColor=tone)), Paragraph(html_escape(text), styles["insight_text"])]],
    ...
    return [_section_title("Leitura executiva", "Pontos de atenção para preparar a equipe.", styles), *cards]


def _schedule(events: list[AgendaEvent], starts_at: datetime, styles: dict) -> list:
    story = [_section_title("Programação detalhada", "Horários, locais e equipes envolvidas.", styles)]
    if not events:
        story.append(Paragraph("Sem compromissos programados para esta semana.", styles["empty_day"]))
        return story
    for index in range(7):
        ...
        day_block = [
            Table(
                [[Paragraph(f"{DAY_NAMES[index]}  •  {day_date.strftime('%d/%m')}", styles["day_heading"]), Paragraph(f"{len(day_events)} compromisso(s)", styles["day_total"])]],
...
def _event_card(event: AgendaEvent, styles: dict):
    ...
    participants = ", ".join(
        str(value.get("nome")) for value in (event.participants or [])
    ...
    content = [
        ...
        Paragraph(html_escape(f"{badges}  •  {event.status.value}"), ParagraphStyle("EventMeta", parent=styles["event_meta"], textColor=status_tone)),
        Paragraph(html_escape(f"Local: {event.location or 'Não informado'}"), styles["event_detail"]),
    ...
    table.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), PURPLE_BG if event.representative_presence else colors.white),
        ("BOX", (0, 0), (-1, -1), 0.55, PURPLE if event.representative_presence else LINE),
        ("LINEAFTER", (0, 0), (0, 0), 0.55, PURPLE if event.representative_presence else LINE),
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
def _section_title(title: str, subtitle: str, styles: dict):
    return Table(
        [[Paragraph(html_escape(title), styles["section_title"]), Paragraph(html_escape(subtitle), styles["section_subtitle"])]],
...
def _styles() -> dict:
    ...
    return {
        "hero_title": ParagraphStyle("HeroTitle", parent=base["Title"], fontName="AgendaSans-Bold", fontSize=23, leading=27, textColor=colors.white, alignment=TA_LEFT),
        "hero_period": ParagraphStyle("HeroPeriod", parent=base["BodyText"], fontName="AgendaSans", fontSize=9, leading=12, textColor=colors.HexColor("#DCEBFA")),
        "office": ParagraphStyle("Office", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=11, leading=14, textColor=colors.white),
        "identity": ParagraphStyle("Identity", parent=base["BodyText"], fontName="AgendaSans", fontSize=7.5, leading=10, textColor=colors.HexColor("#BCD0E6")),
        "section_title": ParagraphStyle("SectionTitle", parent=base["Heading2"], fontName="AgendaSans-Bold", fontSize=11, leading=14, textColor=NAVY),
        "section_subtitle": ParagraphStyle("SectionSubtitle", parent=base["BodyText"], fontName="AgendaSans", fontSize=7, leading=10, textColor=MUTED, alignment=TA_LEFT),
        "metric_value": ParagraphStyle("MetricValueBase", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=19, leading=21),
        "metric_label": ParagraphStyle("MetricLabel", parent=base["BodyText"], fontName="AgendaSans", fontSize=6.6, leading=8.5, textColor=INK),
        "day_short": ParagraphStyle("DayShort", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=6.4, leading=8, textColor=MUTED, alignment=TA_CENTER),
        "day_count": ParagraphStyle("DayCount", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=13, leading=15, textColor=NAVY, alignment=TA_CENTER),
        "insight_label": ParagraphStyle("InsightLabelBase", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=6.1, leading=8),
        "insight_text": ParagraphStyle("InsightText", parent=base["BodyText"], fontName="AgendaSans", fontSize=7.2, leading=10, textColor=INK),
        "day_heading": ParagraphStyle("DayHeading", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=8, leading=10, textColor=colors.white),
        "day_total": ParagraphStyle("DayTotal", parent=base["BodyText"], fontName="AgendaSans", fontSize=6.8, leading=9, textColor=colors.HexColor("#D7E5F3"), alignment=TA_CENTER),
        "empty_day": ParagraphStyle("EmptyDay", parent=base["BodyText"], fontName="AgendaSans", fontSize=7.4, leading=11, textColor=MUTED, leftIndent=0.25 * cm, spaceAfter=0.1 * cm),
        "event_time": ParagraphStyle("EventTime", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=9, leading=12, textColor=NAVY, alignment=TA_CENTER),
        "event_title": ParagraphStyle("EventTitle", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=8.2, leading=11, textColor=INK),
        "event_meta": ParagraphStyle("EventMetaBase", parent=base["BodyText"], fontName="AgendaSans-Bold", fontSize=5.8, leading=8, spaceBefore=2),
        "event_detail": ParagraphStyle("EventDetail", parent=base["BodyText"], fontName="AgendaSans", fontSize=6.8, leading=9.5, textColor=MUTED, spaceBefore=1),
        "event_description": ParagraphStyle("EventDescription", parent=base["BodyText"], fontName="AgendaSans", fontSize=6.6, leading=9.3, textColor=INK, spaceBefore=3),
    }
