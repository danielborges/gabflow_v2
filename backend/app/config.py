...
class Config:
    ...
    TERRITORY_OFFICIAL_IMPORT_ENABLED = (
        os.getenv("TERRITORY_OFFICIAL_IMPORT_ENABLED", "true").lower() == "true"
    )
    ...
    RAG_OPERATIONAL_MEMORY_MATERIALIZED_SNAPSHOTS = int(
        os.getenv("RAG_OPERATIONAL_MEMORY_MATERIALIZED_SNAPSHOTS", "5")
    )
    ...
    RAG_STRUCTURED_INTERPRETATION_ENABLED = (
        os.getenv("RAG_STRUCTURED_INTERPRETATION_ENABLED", "true").lower() == "true"
    )
    RAG_STRUCTURED_INTERPRETATION_PROVIDER = os.getenv(
        "RAG_STRUCTURED_INTERPRETATION_PROVIDER", "ollama"
    )
    RAG_STRUCTURED_INTERPRETATION_MODEL = os.getenv(
        "RAG_STRUCTURED_INTERPRETATION_MODEL", "qwen2.5:0.5b"
    )
    RAG_STRUCTURED_INTERPRETATION_TIMEOUT_SECONDS = int(
        os.getenv("RAG_STRUCTURED_INTERPRETATION_TIMEOUT_SECONDS", "45")
    )
    RAG_STRUCTURED_INTERPRETATION_MAX_TOKENS = int(
        os.getenv("RAG_STRUCTURED_INTERPRETATION_MAX_TOKENS", "128")
    )
    ...
    AI_PRECEDENT_FALLBACK_SCORE_THRESHOLD = float(
        os.getenv("AI_PRECEDENT_FALLBACK_SCORE_THRESHOLD", "0.20")
    )
    ...
    AI_PRECEDENT_SEMANTIC_CANDIDATE_LIMIT = max(
        int(os.getenv("AI_PRECEDENT_SEMANTIC_CANDIDATE_LIMIT", "20")),
        1,
    )
...
class TestConfig(Config):
    ...
    TERRITORY_OFFICIAL_IMPORT_ENABLED = False
    GEOAPIFY_API_KEY = ""
    SENTRY_DSN = None
    RAG_OPERATIONAL_MEMORY_ENABLED = False
    RAG_STRUCTURED_INTERPRETATION_ENABLED = False
