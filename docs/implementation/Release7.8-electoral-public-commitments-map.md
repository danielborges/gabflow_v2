...
Em 4 de agosto de 2026, os fluxos de criação, atualização, evidência append-only, prazo
vencido derivado, supressão e mapa oficial foram aprovados nos testes eleitorais de backend.
Os componentes de lista e mapa foram aprovados na suíte frontend, incluindo sincronização e
rejeição de ponto não confirmado como público. A migração PostgreSQL permanece no gate do
pipeline, junto da migração de fechamento do Incremento 5; ambas passaram localmente pelos nove
testes de upgrade, rollback, PostGIS, grants e RLS com roles reais de runtime.
